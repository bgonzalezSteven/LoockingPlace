<template>
  <q-page class="q-pa-sm" style="min-height: 400px">
    <div class="row text-bold items-center justify-center">
      <q-chip square>
        <q-avatar color="red" text-color="white">
          <template v-if="counter.count > 0">{{counter.count}}</template>
          <template v-else>
          <q-icon name="done"/></template>
        </q-avatar>
        {{counter.message}}
      </q-chip>
    </div>
          ...
          <vs-table dark>
            ...
            <template dense #tbody>
              <vs-tr
                ...
                :data="tr"
              >
                <vs-td>{{ tr.typeRoom.roomsType }}</vs-td>
                <vs-td>{{ tr.range[0] }} - {{tr.range[tr.range.length -1 ] }}</vs-td>
...
export default {
  mounted() {
    this.infoSolicity();
    this.countRoom();
    this.$emit("setTittle", "Cantidad de Habitaciones");
  },
  data() {
    return {
      date: [],
  ...
  methods: {
    async countRoom() {
      this.$q.loading.show();
      this.$api.get("counterRoom").then((res) => {
        this.counter = res;
        this.$q.loading.hide();
      });
    },
